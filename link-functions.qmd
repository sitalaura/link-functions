...
::: notes
link=identity significa che incremento unitario in x, costante in y
...
cosa vuol dire cosa vuol dire applicare il link: trasformare da non lineare a lineare.

per guardare l’interazione non posso guardare il grafico a sinistra, ma dal grafico a destra dove è avvuta la trasformazione che ho chieston al mio glm di fare, applicando il logit all'accuracy (facendo il logaritmo di eta): cosi ottengo y-trasformata, che va da -inf a +inf. quindi, una volta individuata la funzione di link, dobbiamo fare un passo indietro e applicare la funzione inversa: trasformiamo y in modo tale che vada da -inf a +inf e quindi possiamo osservare se le rette (che abbiamo quindi ottenuto linearizzando il fenomeno) sono parallele o meno. 
ogni link esegue una trasformazione diversa. di conseguenza, link diverse linearizzano in modo diverso. solo la l'inversa della link corretta linearizza bene, mentre le inverse delle link sbagliate trovano interazioni spurie 
:::
